...
StringEager::StringEager(const StringEager& rhs)
    : start_(kEmpty_), size_(rhs.size_), capacity_(0) {
    init(rhs.start_);
}
...
StringEager::StringEager(StringEager&& rhs)
    : start_(rhs.start_), size_(rhs.size_), capacity_(rhs.capacity_) {
    rhs.start_ = NULL;
}
...
StringEager::StringEager(const char* str)
    : start_(kEmpty_), size_(::strlen(str)), capacity_(0) {
    init(str);
}

StringEager::StringEager(const char* str, size_t len)
    : start_(kEmpty_), size_(len), capacity_(0) {
    init(str);
}
...
StringEager::StringEager(char* str, uint32_t sz, uint32_t cap, NoAlloc)
    : start_(str), size_(sz), capacity_(cap) {}
...
void StringEager::append(const char* str) { append(str, ::strlen(str)); }
