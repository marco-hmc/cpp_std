...
class InputFile : boost::noncopyable {
  public:
    InputFile(const char* filename) : file_(fopen(filename, "rb")) {
        assert(file_);
...
class OutputFile : boost::noncopyable {
  public:
    OutputFile(const char* filename) : file_(fopen(filename, "wb")) {
        assert(file_);
        setbuffer(file_, buffer_, sizeof buffer_);
    }

    ~OutputFile() { fclose(file_); }
...
int main(int argc, char* argv[]) {
    ...
    // output
    {
        ...
        for (std::vector<string>::iterator it = data.begin(); it != data.end();
             ++it) {
            out.writeLine(*it);
        }
    }
