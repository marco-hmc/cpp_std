...
#### 2.1 STL 中内存分配器设计为模板参数而非构造函数参数的原因
将 STL 中的内存分配器设计为模板参数，而非构造函数参数，主要有以下几点原因：
 - **编译期确定**：模板参数在编译期确定，这允许编译器根据不同的分配器类型进行更优化的代码生成。如果是构造函数参数，只有在运行时才能确定，编译器难以在编译期针对特定分配器进行优化。
 - **类型相关特性**：不同的内存分配器可能具有不同的类型相关特性，如对齐要求等。模板参数可以在编译期确保容器和分配器类型之间的正确匹配，避免运行时错误。
 - **提高效率**：模板参数使得编译器可以在编译期展开代码，减少运行时的间接性，从而提高性能。相比之下，构造函数参数会引入运行时的额外开销。
 - **代码复用与灵活性**：模板参数的设计让同一个容器可以方便地搭配不同的分配器，实现不同的内存管理策略，提高了代码的复用性和灵活性。

#### 2.2 STL 空间配置器处理内存的方式
STL 的空间配置器负责管理容器的内存分配与释放。默认分配器`std::allocator`直接利用`new`和`delete`操作符来进行内存的分配与释放。但在面对大量小对象时，这种方式易产生内存碎片，影响性能。
...
```
这里通过`reserve`方法预先分配了`output`容器所需的空间，避免了在`std::transform`过程中因不断增长容器而频繁进行内存重新分配，提升了性能。

#### 2.4 对 STL 分配器及两级分配器的理解
STL 的分配器是用于控制内存分配和释放的接口。所有 STL 容器类都接受一个分配器类型作为模板参数，方便用户自定义内存管理策略。

SGI STL 引入的两级分配器，针对不同内存需求采用不同策略：
 - **一级分配器**：当请求分配的内存大于 128 字节时，直接使用`malloc`和`free`进行内存的分配与释放。
 - **二级分配器**：当请求分配的内存小于等于 128 字节时，利用内存池管理内存。先预先分配一大块内存，再分割为多个小块供程序使用，以此减少内存分配和释放的开销，降低内存碎片的产生。

需注意，这只是 SGI STL 的实现方式，不同的 STL 实现可能采用不同的内存管理策略。 
