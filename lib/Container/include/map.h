...
namespace MyStd::MapImpl1 {
    ...
    struct RbTreeValueCompare {
      ...
      public:
        ...
        bool operator()(Value const &_lhs, Value const &_rhs) const noexcept {
    ...
    struct Map
        : RbTreeImpl<std::pair<Key const, Mapped>,
      ...
      public:
        ...
        using typename RbTreeImpl<value_type, ValueComp, Alloc>::const_iterator;
        ...
        explicit Map(std::initializer_list<value_type> _ilist, Compare _comp)
            : RbTreeImpl<value_type, ValueComp, Alloc>(_comp) {
            _M_single_insert(_ilist.begin(), _ilist.end());
        }
        ...
        iterator find(Key const &_key) noexcept { return this->_M_find(_key); }
        ...
        template <class Mp,
                  class = std::enable_if_t<std::is_convertible_v<Mp, Mapped>>>
        ...
        template <class Mp,
                  class = std::enable_if_t<std::is_convertible_v<Mp, Mapped>>>
        std::pair<iterator, bool> insert_or_assign(Key &&_key, Mp &&_mapped) {
            std::pair<iterator, bool> _result = this->_M_single_emplace(
        ...
        size_t erase(Key const &_key) { return this->_M_single_erase(_key); }
    ...
    struct MultiMap
      ...
      public:
        using typename RbTreeImpl<value_type, ValueComp, Alloc>::iterator;
        using
            typename RbTreeImpl<value_type, ValueComp, Alloc>::const_iterator;
        using typename RbTreeImpl<value_type, ValueComp, Alloc>::node_type;
        ...
        MultiMap(MultiMap const &other)
            : RbTreeImpl<value_type, ValueComp, Alloc>() {
        ...
        template <class... Ts>
        std::pair<iterator, bool> try_emplace(Key const &_key, Ts &&..._value) {
            return this->_M_single_emplace(
                std::piecewise_construct, std::forward_as_tuple(_key),
                std::forward_as_tuple(std::forward<Ts>(_value)...));
        }
