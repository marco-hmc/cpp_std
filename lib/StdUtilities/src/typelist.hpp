 ...
 */
    ...
    struct largest_type<List, false> {
      ...
      public:
        using type = std::conditional_t<(sizeof(contender) >= sizeof(best)),
                                        contender, best>;
    };
 ...
 */
    ...
    struct insert_sorted<List, Element, Compare, false> {
      private:
        ...
        using new_head =
            std::conditional_t<Compare<Element, front_t<List>>::value, Element,
                               front_t<List>>;
    ...
    template <typename List, typename Element,
              template <typename T, typename U> class Compare>
    using insert_sorted_t =
        typename insert_sorted<List, Element, Compare>::type;
...
namespace jc::test {
    ...
    static_assert(
        std::is_same_v<integral_constant_typelist<std::size_t, 2, 3, 5>,
    ...
    static_assert(
        std::is_same_v<transform_t<T3, std::add_const>,
    ...
    static_assert(
        accumulate_t<integral_constant_typelist<int, 2, 3, 5>, multiply,
    ...
    static_assert(
        std::is_same_v<insertion_sort_t<ConsList, smaller>,
