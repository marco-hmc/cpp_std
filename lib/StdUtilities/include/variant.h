    ...
    namespace VariantImpl1 {
        ...
        struct Variant {
          private:
            ...
            alignas(std::max(
                {alignof(Ts)...})) char m_union[std::max({sizeof(Ts)...})];
            ...
            using CopyConstructorFunction = void (*)(char *,
                                                     char const *) noexcept;

            static CopyConstructorFunction *copy_constructors_table() noexcept {
                static CopyConstructorFunction function_ptrs[sizeof...(Ts)] = {
                    [](const char *union_dst, char const *union_src) noexcept {
                        new (union_dst)
                            Ts(*reinterpret_cast<Ts const *>(union_src));
                    }...};
                return function_ptrs;
            }

            using CopyAssignmentFunction = void (*)(char *,
                                                    char const *) noexcept;

            static CopyAssignmentFunction *
            copy_assigment_functions_table() noexcept {
                static CopyAssignmentFunction function_ptrs[sizeof...(Ts)] = {
                    [](char *union_dst, char const *union_src) noexcept {
                        *reinterpret_cast<Ts *>(union_dst) =
                            *reinterpret_cast<Ts const *>(union_src);
                    }...};
                return function_ptrs;
            }
            ...
            static MoveConstructorFunction *move_constructors_table() noexcept {
                static MoveConstructorFunction function_ptrs[sizeof...(Ts)] = {
                    [](const char *union_dst, const char *union_src) noexcept {
                        new (union_dst) Ts(std::move(
                            *reinterpret_cast<Ts const *>(union_src)));
                    }...};
                return function_ptrs;
            }
            ...
            static MoveAssignmentFunction *
            move_assigment_functions_table() noexcept {
            ...
            using ConstVisitorFunction =
                std::common_type<typename std::invoke_result<
                    Lambda, Ts const &>::type...>::type (*)(char const *,
                                                            Lambda &&);

            template <class Lambda>
            static ConstVisitorFunction<Lambda> *
            const_visitors_table() noexcept {
                static ConstVisitorFunction<Lambda>
                    function_ptrs[sizeof...(Ts)] = {
                        [](char const *union_p, Lambda &&lambda) ->
                        typename std::invoke_result<Lambda, Ts const &>::type {
            ...
            static VisitorFunction<Lambda> *visitors_table() noexcept {
                static VisitorFunction<Lambda> function_ptrs[sizeof...(Ts)] = {
                    [](const char *union_p, Lambda &&lambda)
                        -> std::common_type<typename std::invoke_result<
                            Lambda, Ts &>::type...>::type {
          ...
          public:
            ...
            Variant &operator=(Variant const &that) {
                ...
                copy_assigment_functions_table()[index()](m_union,
                                                          that.m_union);
            }
            ...
            Variant &operator=(Variant &&that) {
                m_index = that.m_index;
                move_assigment_functions_table()[index()](m_union,
                                                          that.m_union);
            }

            template <size_t I, typename... Args>
            explicit Variant(InPlaceIndex<I> /*unused*/, Args &&...value_args)
                : m_index(I) {
            ...
            template <class Lambda>
            std::common_type<
                typename std::invoke_result<Lambda, Ts const &>::type...>::type
            ...
            typename VariantAlternative<Variant, I>::type &get() {
                ...
                if (m_index != I) throw BadVariantAccess();
                return *reinterpret_cast<
                    typename VariantAlternative<Variant, I>::type *>(m_union);
            }
            ...
            typename VariantAlternative<Variant, I>::type const &get() const {
                ...
                if (m_index != I) throw BadVariantAccess();
                return *reinterpret_cast<
                    typename VariantAlternative<Variant, I>::type const *>(
                    m_union);
            }
            ...
            template <size_t I>
            typename VariantAlternative<Variant, I>::type const *get_if()
                const {
                static_assert(I < sizeof...(Ts), "I out of range!");
                if (m_index != I) return nullptr;
                return reinterpret_cast<
        ...
        struct VariantAlternative<Variant<T, Ts...>, I> {
            using type =
                typename VariantAlternative<Variant<Ts...>, I - 1>::type;
        };
        ...
        struct VariantIndex<Variant<T0, Ts...>, T> {
            static constexpr size_t value =
                VariantIndex<Variant<Ts...>, T>::value + 1;
        };
